...
function UserListRow({ user, refetchFavoriteUsers, favoriteUsers, refetchAllUsers }) {
  ...
  return (
    <tr className="hover:bg-gray-50 md:p-0">
      <th className="flex gap-3 p-2 md:p-4 font-normal text-gray-900">
      ...
      <td className="p-2 md:p-4 text-xs md:text-sm">{user?.profession}</td>
      ...
      {userInfo.isAdmin ? (
        <td className="p-2 md:p-4 text-xs md:text-sm">
          <div className="flex justify-end gap-4">
            ...
            <Link x-data="{ tooltip: 'Edit' }" to={`/updateuser/${user._id}`} state={{ user }}>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                fill="none"
                viewBox="0 0 24 24"
        ...
        !isFavorite
          ? <td><MdBookmarkAdd onClick={() => handleAddUserToFavorite(user._id)} className='mb-2 ml-0 md:ml-10 cursor-pointer h-6 w-6' title='add to favorites' /></td>
          : <td><MdBookmarkAdded onClick={() => handleRemoveUserFromFavorites(user._id)} className='mb-2 ml-0 md:ml-10 cursor-pointer h-6 w-6' title='remove from favorites' /></td>
      )}
    </tr>
  );
}
